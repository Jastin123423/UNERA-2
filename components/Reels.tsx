...
export const ReelsFeed: React.FC<ReelsFeedProps> = ({ reels, users, currentUser, onProfileClick, onCreateReelClick, onLoadMore, onReact, onComment, onShare, onFollow, getCommentAuthor, initialReelId }) => {
    ...
    useEffect(() => {
        Object.keys(videoRefs.current).forEach((key) => {
            ...
            if (video) {
                const shouldPlay = id === activeReelId;
                if (shouldPlay) {
    ...
    const handleVideoClick = (reelId: number, video: HTMLVideoElement) => {
        if (video.paused) {
            video.play().catch(() => {});
        } else {
            video.pause();
    ...
    return (
        <div className="w-full h-[calc(100vh-56px)] flex justify-center bg-[#18191A] overflow-hidden relative font-sans">
            ...
            <div ref={containerRef} className="w-full h-full overflow-y-scroll snap-y snap-mandatory scrollbar-hide scroll-smooth">
                {reels.map((reel, index) => {
                    ...
                    const hasLiked = currentUser && reel.reactions.some(r => r.userId === currentUser.id);

                    if (!author) return null;
                    return (
                        <div key={`${reel.id}-${index}`} data-reel-id={reel.id} data-index={index} className="reel-container w-full h-full snap-start relative bg-black flex items-center justify-center overflow-hidden" onDoubleClick={() => handleDoubleTap(reel.id)}>
                            <video ref={el => { if (el) videoRefs.current[reel.id] = el; else delete videoRefs.current[reel.id]; }} src={reel.videoUrl} className="w-full h-full object-cover" loop muted={isMuted} playsInline onTimeUpdate={() => handleTimeUpdate(reel.id)} onClick={(e) => handleVideoClick(reel.id, e.currentTarget)} crossOrigin="anonymous"/>
                            
                            <div className="absolute bottom-0 left-0 w-full h-1 bg-gray-600 z-30"><div className="h-full bg-[#1877F2] transition-all duration-100 ease-linear" style={{ width: `${videoProgress[reel.id] || 0}%` }}></div></div>
                            ...
                            {/* View Counter with Blinking Eye */}
                            <div className="absolute top-4 left-4 z-40 bg-black/40 backdrop-blur-md px-3 py-1.5 rounded-full flex items-center gap-2 border border-white/10 shadow-lg pointer-events-auto">
                                <i className="fas fa-eye text-white text-sm animate-blink"></i>
                                <span className="text-white text-sm font-black tracking-tight">{(reel.views || (reel.id * 153)).toLocaleString()}</span>
                            </div>
                            ...
                            {/* Professional Redesigned Sidebar */}
                            <div className="absolute right-4 bottom-28 flex flex-col items-end gap-3 z-40 pointer-events-auto">
                                <div className="flex items-center gap-3 cursor-pointer group" onClick={() => onReact(reel.id, 'like')}>
                                    <div className={`w-16 h-16 bg-black/30 backdrop-blur-md rounded-full flex items-center justify-center border border-white/10 transition-all ${hasLiked ? 'text-[#F3425F]' : 'text-white'}`}>
                                        <i className={`${hasLiked ? 'fas' : 'far'} fa-heart text-4xl group-active:scale-125`}></i>
                                    </div>
                                    <span className="text-white text-[18px] font-black drop-shadow-md min-w-[32px] text-left">{reel.reactions?.length || 0}</span>
                                </div>
                                <div className="flex items-center gap-3 cursor-pointer group" onClick={() => onComment(reel.id, 'open')}>
                                    <div className="w-16 h-16 bg-black/30 backdrop-blur-md rounded-full flex items-center justify-center border border-white/10 text-white transition-all">
                                        <i className="far fa-comment-dots text-4xl group-active:scale-110"></i>
                                    </div>
                                    <span className="text-white text-[18px] font-black drop-shadow-md min-w-[32px] text-left">{reel.comments?.length || 0}</span>
                                </div>
                                <div className="flex items-center gap-3 cursor-pointer group" onClick={() => onShare(reel.id, 'feed')}>
                                    <div className="w-16 h-16 bg-black/30 backdrop-blur-md rounded-full flex items-center justify-center border border-white/10 text-white transition-all">
                                        <i className="fas fa-share text-4xl group-active:scale-110"></i>
                                    </div>
                                    <span className="text-white text-[18px] font-black drop-shadow-md min-w-[32px] text-left">{reel.shares || 0}</span>
                                </div>
                            </div>

                            <div className="absolute bottom-0 left-0 w-full p-4 z-20 pb-10 bg-gradient-to-t from-black/90 via-black/40 to-transparent pointer-events-none">
                                <div className="flex items-center gap-3 mb-3 pointer-events-auto cursor-pointer" onClick={() => onProfileClick(author.id)}>
                                    <div className="relative">
                                        ...
                                        {!isFollowing && !isSelf && currentUser && <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-red-500 rounded-full flex items-center justify-center border border-white" onClick={(e) => { e.stopPropagation(); onFollow(author.id); }}><i className="fas fa-plus text-white text-[8px]"></i></div>}
                                    </div>
